...
from src.services.notification_template import render_template

# 默认路由表：event_type → [channels]
DEFAULT_ROUTES: dict[str, list[str]] = {
    # 用户域
    "user.registered": ["email"],
    "user.password_reset": ["email"],
    "user.password_changed": ["email"],
    "user.profile_updated": ["email"],
    "user.status_changed": ["email", "dingtalk"],
    "user.login_failed": ["email", "dingtalk"],
    # 角色权限域
    "role.assigned": ["email", "dingtalk"],
    "permission.granted": ["email"],
    "permission.revoked": ["email", "dingtalk"],
    # 文件域
    "file.uploaded": ["email"],
    "file.shared": ["email", "dingtalk"],
    # 系统域
    "system.alert": ["email", "dingtalk", "feishu"],
    "system.maintenance": ["email", "dingtalk", "feishu"],
}
...
class NotificationDispatcher:
    ...
    不继承 BaseService —— 不是 CRUD 服务，而是事件分发协调器。
    """
    ...
    def dispatch(
        ...
        Args:
            event_type: 事件类型（字符串或枚举），如 "user.registered"
        ...
        target_channels = channels or DEFAULT_ROUTES.get(event_type_str, ["email"])
        ...
        for channel in target_channels:
            ...
            if provider is None:
                logger.warning(
                    "No provider registered for channel=%s, skipping", channel
                )
                continue

            subject, content = render_template(event_type_str, channel, variables)
            ...
            message = NotificationMessage(
                ...
                subject=subject,
                content=content,
                content_type="html" if channel == "email" else "text",
